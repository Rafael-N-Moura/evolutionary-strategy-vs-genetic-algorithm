...
for nome, func in FUNCOES.items():
    print(f'Executando GA para {nome}...')
    melhores_fitness = []
    historicos = []
    for execucao in range(NUM_EXECUCOES):
        ...
        melhor_ind, melhor_fit, hist = ga.run()
    ...
    np.savetxt(f'resultados/ga_{nome}_melhores_fitness.txt', melhores_fitness)
    # Salvar histórico médio e desvio padrão
    media_hist = np.mean(historicos, axis=0)
    std_hist = np.std(historicos, axis=0)
    np.savetxt(f'resultados/ga_{nome}_convergencia_media.txt', media_hist)
    np.savetxt(f'resultados/ga_{nome}_convergencia_std.txt', std_hist)
    # Salvar estatísticas
    with open(f'resultados/ga_{nome}_estatisticas.txt', 'w') as f:
    ...
    plt.title(f'Convergência Média GA - {nome.capitalize()}')
    plt.legend()
    plt.grid()
    plt.savefig(f'resultados/ga_{nome}_convergencia_media.png')
    plt.close()
    # Gerar boxplot dos melhores fitness
    plt.figure()
    plt.boxplot(melhores_fitness)
    plt.ylabel('Melhor Fitness')
    plt.title(f'Boxplot dos Melhores Fitness - GA - {nome.capitalize()}')
    plt.grid()
    plt.savefig(f'resultados/ga_{nome}_boxplot.png')
    plt.close()
